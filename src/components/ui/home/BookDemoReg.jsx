...
//   // Get subject from URL state or localStorage
//   const subjectFromState = locationHook.state?.subject || localStorage.getItem("selectedSubject");

//   // Initialize selectedSubjects with the subject from navigation
//   useEffect(() => {
...
//       const hasMaxPrice = form.maxPrice !== undefined && form.maxPrice !== null && form.maxPrice !== "";
...
//   try {
//     let finalSubjects = [];
//     if (locationHook.state?.subject) {
//       finalSubjects = [locationHook.state.subject];
//     } else if (selectedSubjects?.length > 0) {
//       finalSubjects = selectedSubjects;
//     }

//     // Convert modes to backend format
//     const classModesArray = Object.keys(selectedModes)
//       .filter((k) => selectedModes[k])
//       .map((mode) => mode === "online" ? "Online" : "Offline");
...
//       availability: [availability], // backend wants array
//       start_timeline: startTimeline,
//       class_modes: classModesArray,
//       tutor_gender_preference: genderPref,
//       hourly_charges: Number(form.minPrice),
//       place_id: location?.place_id || "",
//       school_name: "St. Xavier's High School", // placeholder
//       languages: [
...
// // In your apiClient or authRepository
// const preRegisterStudent = async (payload) => {
//   try {
//     console.log("API Request:", JSON.stringify(payload, null, 2));
//     const response = await apiClient.post('/student/pre-register', payload);
...
//     setLoading(true);
//     try {
//       await authRepository.verifyPreRegisterOtp({
...
//     } catch (err) {
//       console.error(err);
//       setErrors({ submit: err?.response?.data?.message || "OTP verification failed." });
//     } finally {
//       setLoading(false);
//     }
//   };
...
//                       }}
//                       placeholder="Min (₹)"
//                       className={`w-full border ${errors.minPrice ? "border-red-500" : "border-gray-300"} rounded-lg p-2 focus:ring-2 focus:ring-teal-500 focus:border-transparent`}
//                     />
...
//                       }}
//                       placeholder="Max (₹)"
//                       className={`w-full border ${errors.maxPrice ? "border-red-500" : "border-gray-300"} rounded-lg p-2 focus:ring-2 focus:ring-teal-500 focus:border-transparent`}
//                     />
...
//           <div className="mt-6 p-4 border border-gray-200 rounded bg-gray-50 text-xs text-gray-600">
//             <pre>{JSON.stringify({ step, location, board, boardOtherText, selectedSubjects, availability, startTimeline, selectedModes, genderPref, form, otp }, null, 2)}</pre>
//           </div>
//         </div>
//       </div>
//     </Layout>
          ...
          {step === 8 && (
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={form.maxPrice || ""}
                      onChange={(e) =>
                        setForm({ ...form, minPrice: 100, maxPrice: e.target.value })
                      }
              ...
              <div className="flex justify-between mt-6">
                ...
                <button
                  onClick={() => {
                    const maxPriceValue = form.maxPrice ? Number(form.maxPrice) : 100;

                    if (maxPriceValue < 100) {
                      setErrors({
                        ...errors,
                        maxPrice: "Max price must be at least ₹100",
                      });
                      return;
                    }

                    setForm({ ...form, minPrice: 100, maxPrice: maxPriceValue });
                    goNext();
                  }}
