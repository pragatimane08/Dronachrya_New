...
//         toast.success("OTP verified successfully!", {
//           autoClose: 2000,
//           onClose: () => navigate("/tutor_referral_code", { state: { token } }),
//         });
//       } else {
//         toast.error("⚠ OTP verification failed.");
//       }
...
const TutorReg = ({ onClose }) => {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      const { token, user } = res.data;
      if (token) {
        // For tutor role, we need to perform a password login after OTP verification
        // This is likely what's causing the 400 error
        try {
          // Perform a password login for tutors after OTP verification
          const loginRes = await authRepository.login({
            email: formData.email.trim(),
            password: formData.password.trim(),
            role: "tutor"
          });
          
          const { token: authToken, user: authUser } = loginRes.data;
          ...
          toast.success("OTP verified successfully!", {
            autoClose: 2000,
            onClose: () => navigate("/tutor_referral_code", { state: { token: authToken } }),
          });
        } catch (loginError) {
          // If password login fails, fall back to the token from OTP verification
          localStorage.setItem("authToken", token);
          localStorage.setItem("user", JSON.stringify(user));
          localStorage.setItem("role", user.role);
          localStorage.setItem("user_id", user.id);
      ...
      // If the error is about requiring password login, try to login with password
      if (msg.includes("must log in with password")) {
        try {
          const loginRes = await authRepository.login({
            email: formData.email.trim(),
            password: formData.password.trim(),
          ...
          const loginMsg = loginError.response?.data?.message || "❌ Login failed.";
