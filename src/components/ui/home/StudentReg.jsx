...
//     if (!emailRegex.test(formData.email.trim())) errors.email = "Enter a valid email.";
//     if (!phoneRegex.test(formData.phone.trim())) errors.phone = "Enter a 10-digit mobile number.";
...
//       errors.password = "Password must be 6+ chars with uppercase, lowercase, digit & special char.";
//     }
...
//       subjects: formData.subject.split(",").map((s) => s.trim()).filter(Boolean),
//       referralCode: formData.referralCode?.trim() || null, // ✅ Added
//     };
...
//           autoClose: 1500,
//           onClose: () => navigate("/location-selector", { state: { token } }),
//         });
//       } else {
...
//                 { label: "Password", name: "password", type: "password", placeholder: "Student@123" },
//                 { label: "Referral Code (optional)", name: "referralCode", type: "text", placeholder: "ABC123" }, // ✅ Added
//               ].map((input) => (
//                 <div key={input.name}>
...
//                     required={input.name !== "referralCode"} // not required for referral
...
//                     setOtp(newOtp);
//                     if (newOtp.length === 6 && otp.length < 6) {
//                       toast.info("ℹ OTP entered. Click Verify to continue.");
//                     }
//                   }}
