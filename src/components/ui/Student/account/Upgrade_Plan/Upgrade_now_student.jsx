...
import { apiClient, getToken, validateToken, isAuthenticated, getUser } from "../../../../../api/apiclient";
...
const SubscriptionPlans = ({ userType = "student" }) => {
  ...
  // Fixed calculateDiscountPercentage function - ensures it never exceeds 100%
  const calculateDiscountPercentage = (baseAmount, discountAmount) => {
    if (!baseAmount || baseAmount <= 0 || !discountAmount || discountAmount <= 0) {
      return 0;
    }
    const percentage = (discountAmount / baseAmount) * 100;
    // Cap the percentage at 100% to avoid showing more than 100% OFF
  ...
  const getDisplayAmounts = () => {
    ...
    const baseAmount = parseFloat(selectedPlan.price);
    const discountAmount = paymentSummary?.discount_amount || 0;
    
    // Calculate discount percentage based ONLY on base amount (excluding GST)
    const discountPercentage = calculateDiscountPercentage(baseAmount, discountAmount);
    ...
    return {
      ...
      discountedBase: discountedBase.toFixed(2),
  ...
  useEffect(() => {
    const checkAuthPeriodically = async () => {
      ...
      if (!authStatus.authenticated && showModal) {
        showInlineMessage("Your session has expired. Please login again.", "error");
        setShowModal(false);
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      ...
      try {
        ...
        const res = await apiClient.get(apiUrl.subscriptions.getPlans(userType));
        ...
        if (err.response?.status === 401) {
          ...
          showInlineMessage("Unable to load subscription plans. Please try again later.", "error");
        }
      } finally {
    ...
    fetchPlans();
  }, [userType, isAuthenticatedState, navigate]);
  ...
  const calculateOrder = async (couponToApply = null, orderId = null) => {
    ...
    try {
      ...
      const currentCouponCode = couponToApply || (appliedCoupon ? appliedCoupon.code : null);
      ...
      if (error.response?.data?.message) {
        ...
        if (errorMsg.includes("coupon") || errorMsg.includes("Coupon")) {
          ...
          showInlineMessage(errorMsg || "Failed to calculate order. Please try again.", "error");
        }
      } else {
        showInlineMessage("Failed to calculate order. Please try again.", "error");
      }
      return null;
    } finally {
  ...
  // Fixed handleApplyCoupon function with proper percentage calculation
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      const orderSummary = await calculateOrder(couponCode.trim(), currentOrderId);

      if (orderSummary) {
        // Calculate discount percentage based ONLY on base amount
        ...
        if (orderSummary.discount_amount > 0) {
          setCouponMessage(
            `🎉 Coupon applied! You saved ₹${parseFloat(
              orderSummary.discount_amount
            ).toFixed(2)} (${discountPercentage}% OFF)`
          );
        } else {
      ...
      const errorMessage = error.message || "❌ Error applying coupon. Please try again.";
  ...
  const handlePayment = useCallback(
    async (plan) => {
      ...
      if (!authStatus.authenticated || !authStatus.userData) {
        showInlineMessage("⚠️ Your session has expired. Please login again.", "error");
        setShowModal(false);
        setTimeout(() => navigate("/login"), 2000);
        return;
      }
      ...
      if (!planToUse) {
        showInlineMessage("⚠ Please select a subscription plan before proceeding.", "warning");
        return;
      }
      ...
      try {
        ...
        const userId = extractUserId();
        if (!userId) {
          showInlineMessage("Unable to identify user. Please logout and login again.", "error");
          setShowModal(false);
          setTimeout(() => {
            localStorage.clear();
            navigate("/login");
        ...
        if (paymentSummary && paymentSummary.order_id) {
          console.log("Using existing order for payment:", paymentSummary.order_id);
          await processPaymentWithOrder(paymentSummary, planToUse, userId, authStatus.userData);
          return;
        }
        ...
        await processPaymentWithOrder(orderSummary, planToUse, userId, authStatus.userData);

      } catch (error) {
        console.error("Payment initiation error:", error);
        if (error.response?.status === 401) {
          showInlineMessage("⚠️ Your session has expired. Redirecting to login...", "error");
          setShowModal(false);
          setTimeout(() => navigate("/login"), 2000);
        } else if (error.response?.status === 400) {
          // Handle coupon already applied error specifically
          const errorMsg = error.response?.data?.message || "Coupon already used in pending payment";
          if (errorMsg.includes("already applied") || errorMsg.includes("pending payment")) {
            showInlineMessage("This coupon is already associated with a pending payment. Please remove the coupon and try again.", "error");
            // Reset coupon state
            setAppliedCoupon(null);
            setCouponCode("");
      ...
      handler: async function (response) {
        try {
          ...
          if (!currentAuthStatus.authenticated) {
            showInlineMessage("Session expired during payment. Please contact support.", "error");
            return;
          }
          ...
          showInlineMessage("🎉 Payment Successful! Subscription Activated.", "success");
          // Reset all states
          setAppliedCoupon(null);
          setCouponCode("");
          ...
          if (error.response?.status === 401) {
            showInlineMessage("Session expired during payment verification. Please contact support.", "error");
          } else {
            showInlineMessage(
              error.response?.data?.message ||
  ...
  const getMessageStyles = (type) => {
    const baseStyles = "p-3 rounded-lg text-sm font-medium mb-4 transition-all duration-300";
    switch (type) {
      case "success":
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center px-3 sm:px-4 py-4 sm:py-8">
      ...
      <div className="bg-white rounded-xl border border-gray-300 p-4 sm:p-6 w-full max-w-6xl relative mx-auto">
        ...
        {/* Header */}
        <h2 className="text-center text-[#0E2D63] text-xl sm:text-2xl font-semibold mb-2">
          {userType === "tutor"
        ...
        <p className="text-center text-gray-600 text-xs sm:text-sm mb-6 sm:mb-8 px-2">
          {userType === "tutor"
            ? "Boost your teaching career with flexible tutor plans."
            : "Get access to expert tutors and premium learning features with our student plans."}
        </p>
        ...
        {isAuthenticatedState ? (
          <>
            {fetchingPlans ? (
              <div className="flex flex-col sm:flex-row justify-center items-center py-8 sm:py-10 space-y-4 sm:space-y-0">
                ...
                <p className="ml-0 sm:ml-4 text-gray-500 text-sm sm:text-base">Loading subscription plans...</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6">
                  {plans.map((plan) => (
                    ...
                    >
                      ...
                      <button
                        className="w-full py-2 bg-teal-500 hover:bg-teal-600 text-white rounded text-xs sm:text-sm font-medium transition-colors"
                        onClick={(e) => {
                          e.stopPropagation();
                          setSelectedPlan(plan);
                          setShowModal(true);
                ...
                {showModal && selectedPlan && displayAmounts && (
                    ...
                    <div className="bg-white rounded-xl shadow-lg max-w-full sm:max-w-md w-full p-4 sm:p-6 relative mx-2 sm:mx-0 max-h-[90vh] overflow-y-auto">
                      ...
                      <div className="space-y-2 text-gray-700 text-xs sm:text-sm mb-3 sm:mb-4 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>₹{displayAmounts.baseAmount}</span>
                        </div>
                        
                        {/* GST */}
                        <div className="flex justify-between text-gray-600">
                          <span>+ 18% GST:</span>
                          <span>₹{displayAmounts.gstAmount}</span>
                        </div>

                        {/* Discount Section - Only show if there's a discount */}
                        {parseFloat(displayAmounts.discountAmount) > 0 && (
                          <>
                            <div className="border-t border-gray-300 my-2"></div>
                            <div className="flex justify-between text-green-600 font-medium">
                              <span>Discount Applied:</span>
                              <span>-₹{displayAmounts.discountAmount}</span>
                            </div>
                            <div className="flex justify-between text-green-600">
                              <span>GST Adjustment:</span>
                              <span>-₹{(parseFloat(displayAmounts.gstAmount) - (parseFloat(displayAmounts.discountedBase) * 0.18)).toFixed(2)}</span>
                            </div>
                          </>
                        )}
                        ...
                        <div className="flex justify-between text-base sm:text-lg font-bold">
                          ...
                          <span>₹{displayAmounts.totalAmount}</span>
                        </div>
                        
                        {/* Savings Message */}
                        {parseFloat(displayAmounts.discountAmount) > 0 && (
                          <div className="text-green-600 text-xs sm:text-sm text-center font-medium mt-2">
                            You save ₹{displayAmounts.discountAmount} ({displayAmounts.discountPercentage}% OFF)
                          </div>
                        )}
                      </div>
                      ...
                      {!appliedCoupon && !showCouponSection && (
                        <div className="mb-3 sm:mb-4 text-center">
                          <button
                            ...
                            className="text-teal-600 hover:text-teal-700 text-xs sm:text-sm font-medium underline flex items-center justify-center gap-1 mx-auto"
                          >
                            🎁 Apply Coupon Code & Save More
                          </button>
                        </div>
                      )}
                            ...
                            {couponMessage && (
                              <div
                                className={`text-xs sm:text-sm ${
                                  couponMessage.includes("🎉") || couponMessage.includes("✅")
                                    ? "text-green-600"
                                    : "text-red-600"
                                }`}
                              >
                      ...
                      {appliedCoupon && (
                        <div className="mb-3 sm:mb-4 p-3 border border-green-200 rounded-lg bg-green-50">
                          ...
                          {couponMessage && (
                            <div className="text-green-600 text-xs sm:text-sm mt-1">{couponMessage}</div>
                          )}
                        </div>
                      )}
                      ...
                      >
                        {calculatingOrder
                          ...
                          : loadingPlanId
                          ? "Processing..."
                          : `Pay ₹${displayAmounts.totalAmount}`}
                      </button>
                    </div>
                  </div>
                )}
          ...
          <div className="text-center py-8 sm:py-12">
            ...
            <button
              ...
              className="bg-teal-600 text-white px-6 sm:px-8 py-2 sm:py-3 rounded-lg font-semibold hover:bg-teal-700 transition-colors text-sm sm:text-base"
            >
              Login to Continue
            </button>
          </div>
