...
const EnquiryCard = ({
  ...
  const formatLocation = () => {
    // Handle the location data structure from your API
...
const fetchAll = async () => {
  try {
    ...
    const studentEnquiries = allEnquiries.filter(
      (enquiry) => enquiry.sender?.role?.toLowerCase() === "tutor"
    );
    ...
    const processed = studentEnquiries.map((enquiry) => ({
      ...
      // ✅ FIX: Get location from the correct nested path
      location: enquiry.sender?.profile?.Location || null,
      time: enquiry.created_at,
      // Use class_modes from profile or default to offline
      learningMode: enquiry.sender?.profile?.class_modes?.[0] || "offline",
      email: enquiry.sender?.email,
      phone: enquiry.sender?.mobile_number,
      grade: enquiry.class, // Using class as grade
      budget: enquiry.sender?.profile?.hourly_charges ? 
        `₹${enquiry.sender.profile.hourly_charges}/hr` : null,
      sender_id: enquiry.sender?.id,
      receiver_id: enquiry.receiver?.id,
      status: enquiry.status, // Add status for the respond logic
    }));
...
// Updated handleRespond in AllEnquiriesPage
// In AllEnquiriesPage.jsx - Use the correct method names
// In AllEnquiriesPage.jsx
// In AllEnquiriesPage.jsx - Update handleRespond
// In AllEnquiriesPage.jsx - Update handleRespond
const handleRespond = async (enquiry) => {
  try {
    console.log("📞 Responding to enquiry:", enquiry);
    
    // Make sure we have all required parameters
    if (!enquiry.id || !enquiry.sender_id || !enquiry.receiver_id) {
      console.error("❌ Missing required enquiry data:", enquiry);
      toast.error("Cannot start conversation - missing data");
      return;
    }

    // ✅ STEP 1: Accept the enquiry first
    if (enquiry.status !== 'accepted') {
      try {
        console.log(`✅ Accepting enquiry ${enquiry.id}...`);
        await enquiryRepository.acceptEnquiry(
          enquiry.id, 
          "I'm interested in teaching you. Let's discuss the details!"
        );
        console.log(`✅ Enquiry ${enquiry.id} accepted successfully`);
        toast.success("Enquiry accepted! Redirecting to messages...");
        
        // Refresh the enquiries list to show updated status
        fetchAll();
      } catch (acceptError) {
        console.error("❌ Failed to accept enquiry:", acceptError);
        toast.error("Failed to accept enquiry. Please try again.");
        return; // Don't proceed if acceptance fails
      }
    } else {
      console.log("ℹ️ Enquiry already accepted, proceeding to messages...");
    }

    // ✅ STEP 2: Navigate to messages after successful acceptance
    navigate(`/student_message_dashboard?id=${enquiry.id}&sender=${enquiry.sender_id}&receiver=${enquiry.receiver_id}`);
