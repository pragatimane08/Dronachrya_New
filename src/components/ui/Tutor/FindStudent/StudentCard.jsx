...
const StudentCard = ({ student, onConnectionRequest, onViewProfile, onBookmarkToggle }) => {
  const [showContact, setShowContact] = useState(false);
  const [isConnecting, setIsConnecting] = useState(false);
  const [isBookmarked, setIsBookmarked] = useState(student.is_bookmarked || false);
  ...
  const handleBookmark = async () => {
    try {
      ...
      // Call the bookmark toggle API
      if (onBookmarkToggle) {
        const newBookmarkStatus = await onBookmarkToggle(student.user_id, !isBookmarked);
