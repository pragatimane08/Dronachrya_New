...
const FindStudents = () => {
  ...
  // Bookmark toggle API call
  const toggleBookmark = async (studentId, bookmarked) => {
    try {
      const response = await fetch('http://localhost:3000/api/bookmarks/toggle', {
        method: 'POST',
        headers: {
      ...
      // Update the student's bookmark status in the local state
      setStudents(prevStudents =>
        prevStudents.map(student =>
          student.user_id === studentId
            ? { ...student, is_bookmarked: result.is_bookmarked }
            : student
        )
      );
