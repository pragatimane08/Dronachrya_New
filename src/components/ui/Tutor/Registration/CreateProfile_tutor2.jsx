...
const ClassPreferenceForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (modes.length > 0) payload.teaching_modes = modes;

    if (expValue) payload.experience = expValue;
    if (price) payload.pricing_per_hour = Number(price);
    // You can add languages if available: payload.languages = languagesArray;

    try {
      await apiClient.put("/profile/tutor", payload); // Make sure this endpoint exists
